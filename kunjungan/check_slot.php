...
if (!isset($_POST['tanggal']) || !isset($_POST['poli_id']) || !isset($_POST['jam'])) {
...
// Cek apakah sudah ada pasien di sesi dan poli yang sama
if ($poliId !== null) {
    ...
    mysqli_stmt_bind_param($stmt, 'sis', $tanggal, $poliId, $jam);
} else {
    $stmt = mysqli_prepare($koneksi, 
        "SELECT COUNT(*) as count FROM kunjungan 
         WHERE tanggal_kunjungan = ? AND poli_id IS NULL AND jam_kunjungan = ? AND status_kunjungan != 'batal'"
    );
    mysqli_stmt_bind_param($stmt, 'ss', $tanggal, $jam);
}
